	...
	case "geometry":
		{
			...
			for i := range universe.lijst {
				string := fmt.Sprintf("%v\t%v\t%v\t%v\t%v\n", universe.lijst[i].x, universe.lijst[i].y, universe.lijst[i].z, radii[universe.lijst[i].rindex], universe.lijst[i].msat)
	...
	case "m":
		{
			...
			for i := range universe.lijst {
				string := fmt.Sprintf("%v\t%v\t%v\t%v\t%v\t%v\t%v\t%v\n", universe.lijst[i].x, universe.lijst[i].y, universe.lijst[i].z, radii[universe.lijst[i].rindex], universe.lijst[i].msat, universe.lijst[i].m[0], universe.lijst[i].m[1], universe.lijst[i].m[2])
				_, error = file.WriteString(string)
				check(error)
			}
		}
